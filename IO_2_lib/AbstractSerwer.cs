...
namespace IO_2_lib
{
    public abstract class AbstractSerwer
    {
        IPAddress iPAddress;
        int port;
        ...
        public AbstractSerwer(string IP = "127.0.0.1", int port = 9999)
        {
            Construktor(IPAddress.Parse(IP), port);
        }
        ...
        /// <param name="IP">IP address of the server instance.</param>
        /// <param name="port">Port number of the server instance.</param>
        public AbstractSerwer(IPAddress IP, int port = 9999)
        {
            Construktor(IP, port);
        }

        private void Construktor(IPAddress IP, int port)
        {
            running = false;
            iPAddress = IP;
            this.port = port;
